...
(function() {
    ...
    CONFIG.autosave = CONFIG.autosave !== false && CONFIG.autosave !== 'false';
    ...
    function getAnnotationEndpoint() {
        ...
        return `${CONFIG.apiBase}/annotations?${params.toString()}`;
    }

    async function loadPageAnnotations() {
        const endpoint = getAnnotationEndpoint();
        if (!endpoint || !CONFIG.autoLoad) return;
        try {
            isLoadingRemoteAnnotations = true;
            ...
            isLoadingRemoteAnnotations = false;
        }
    }
    ...
    function schedulePersistAnnotations() {
        if (!CONFIG.autosave || CONFIG.readonly || isLoadingRemoteAnnotations) return;
        window.clearTimeout(persistTimer);
        persistTimer = window.setTimeout(savePageAnnotations, CONFIG.saveDelay);
    }
    ...
    function createToolbar() {
        ...
        toolbar.innerHTML = `
            ...
            <div class="${PREFIX}toolbar-mini">
                <div class="${PREFIX}drag-handle ${PREFIX}drag-handle-mini" title="Drag"></div>
                <button id="${PREFIX}maximize-button" class="${PREFIX}tool-button ${PREFIX}chrome-button" title="Show toolbar">
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24"><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21.174 6.812a1 1 0 0 0-3.986-3.987L3.842 16.174a2 2 0 0 0-.5.83l-1.321 4.352a.5.5 0 0 0 .623.622l4.353-1.32a2 2 0 0 0 .83-.497z"/></svg>
                </button>
            </div>
        `;
        ...
        if (CONFIG.readonly) {
            toolbar.querySelectorAll(`.${PREFIX}tool-button[data-tool]:not([data-tool="navigate"])`).forEach(button => {
                button.disabled = true;
                button.title = 'Readonly mode';
            });
        }
    ...
    function setToolbarMinimized(minimized) {
        ...
        if (!tb) return;
        isToolbarMinimized = minimized;
        tb.classList.toggle(`${PREFIX}minimized`, minimized);

        // Keep the toolbar inside the viewport after the size change.
        const rect = tb.getBoundingClientRect();
        if (rect.right > window.innerWidth) {
            tb.style.left = `${Math.max(0, window.innerWidth - rect.width - 4)}px`;
        }
        if (rect.bottom > window.innerHeight) {
            tb.style.top = `${Math.max(0, window.innerHeight - rect.height - 4)}px`;
        }
    }
    ...
    function addStyles() {
        ...
        style.textContent = `
            ...
            /* Minimize / maximize chrome buttons */
            .${PREFIX}tool-button.${PREFIX}chrome-button {
                padding: 4px 6px;
                min-width: 28px;
                min-height: 28px;
            ...
            .${PREFIX}tool-button.${PREFIX}chrome-button:hover {
                opacity: 1;
                background: rgba(255,255,255,0.08);
                border-color: transparent;
            }

            /* Floating minimize button — small square anchored to the toolbar corner,
               doesn't affect toolbar width */
            .${PREFIX}toolbar { position: fixed; }
            .${PREFIX}chrome-button-floating {
                position: absolute;
                top: -8px;
                right: -8px;
                width: 18px;
            ...
            .${PREFIX}chrome-button-floating:hover {
                opacity: 1;
                background: #3a4b5c;
            }
            .${PREFIX}toolbar.${PREFIX}minimized .${PREFIX}chrome-button-floating {
                display: none;
            }

            /* Minimized state: hide full UI, show compact pill */
            .${PREFIX}toolbar-mini { display: none; }
            .${PREFIX}toolbar.${PREFIX}minimized {
                padding: 4px 6px;
            }
            .${PREFIX}toolbar.${PREFIX}minimized .${PREFIX}toolbar-rows {
                display: none;
            }
            .${PREFIX}toolbar.${PREFIX}minimized .${PREFIX}toolbar-mini {
                display: flex;
                align-items: center;
                gap: 4px;
            }
            .${PREFIX}drag-handle-mini {
                margin-right: 0;
            }

            /* On small screens the toolbar is full-width by default —
               shrink to fit when minimized so it doesn't stretch across the screen. */
            @media (max-width: 480px) {
                .${PREFIX}toolbar.${PREFIX}minimized {
                    left: auto;
                    right: 10px;
    ...
    if (document.readyState === 'complete' || document.readyState === 'interactive') {
        ...
        setTimeout(loadPageAnnotations, 150);
    } else {
